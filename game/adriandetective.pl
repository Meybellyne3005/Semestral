...
%
%
%Burt fue amigo de la victima, pero la victima y carl eran enemigos a muerte
%
%  Burt:
%  I was out of town when it happened, and on top of that I didn't even
%  know the guy.
%
% Yo estaba fuera de la ciudad  cuando paso estoy, y ni siquiera conocia al chico
%
%  Carl:
...
%
%
%yo soy inocente, No sé quién lo hizo. Vi arte y Burt manejando por la ciudad entonces.
%
%
...
contradictsPerson(A,B) :- claims(A,AClaims), claims(B,BClaims), contradictsClaims(AClaims,BClaims).

contradictsClaims(Claims,[H|T]) :- contradictsClaims1(Claims,H); contradictsClaims(Claims,T).
contradictsClaims1([H|T],C) :- contradictsClaim(H,C); contradictsClaims1(T,C).
contradictsClaim([Claim1,Who],[Claim2,Who]) :- contradictory(Claim1,Claim2).


claims(art,[[innocent,art],[knewVic,burt],[knewVic,carl]]).
claims(burt,[[outOfTown,burt],[didNotKnowVic,burt]]).
claims(carl,[[didNotKnow,carl]]).
%claims(burt,[[inTown,burt],[knewVic,burt]]). 
%[innocent,carl],.
